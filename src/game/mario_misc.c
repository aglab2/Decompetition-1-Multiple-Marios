...
Gfx *geo_mario_head_rotation(s32 callContext, struct GraphNode *node, UNUSED Mat4 *mtx) {
    ...
    if (callContext == GEO_CONTEXT_RENDER) {
        struct GraphNodeRotation *rotNode = (struct GraphNodeRotation *) node->next;
        struct Camera *camera = gCurGraphNodeCamera ? gCurGraphNodeCamera->config.camera : NULL;

        if (camera && camera->mode == CAMERA_MODE_C_UP) {
...
Gfx *geo_render_mirror_mario(s32 callContext, struct GraphNode *node, UNUSED Mat4 *mtx) {
    ...
    switch (callContext) {
        case GEO_CONTEXT_CREATE:
            init_graph_node_object(&gMirrorMario, NULL, gVec3fZero, gVec3sZero, gVec3fOne);
            break;
...
Gfx *geo_mirror_mario_backface_culling(s32 callContext, struct GraphNode *node, UNUSED Mat4 *mtx) {
    ...
    if (callContext == GEO_CONTEXT_RENDER) {
        ...
        // this might underflow but it doesn't matter
        gDPSetPrimColor(&gfx[0], 255, 255, sColors[obj->oPlayerID - 1].r, sColors[obj->oPlayerID - 1].g, sColors[obj->oPlayerID - 1].b, 255);
        gDPPipeSync(&gfx[1]);
        gDPPipelineMode(&gfx[2], asGenerated->parameter ? G_PM_1PRIMITIVE : G_PM_NPRIMITIVE);
